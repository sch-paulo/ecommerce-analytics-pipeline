...
if section == "Monthly Revenue Trends":
    ...
    df_plot = df.copy()
    df_plot["recency_segment"] = pd.qcut(
        df_plot["recency_score"], q=5, labels=[1, 2, 3, 4, 5]
    )
    df_plot["frequency_monetary_score"] = (
        df_plot["monetary_score"] + df_plot["frequency_score"]
    ) / 2
    df_plot["frequency_monetary_segment"] = pd.qcut(
        df_plot["frequency_monetary_score"], q=5, labels=[1, 2, 3, 4, 5]
    )
    ...
    sns.heatmap(rfm_pivot, annot=True, fmt=".0f", cmap="YlOrRd", linewidths=0.5, ax=ax)
