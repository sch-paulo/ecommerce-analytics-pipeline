...
def clean_and_transform_data(df: pd.DataFrame):
    ...
    df['CustomerID'] = df['CustomerID'].fillna(0).astype(int).astype(str).replace('0', 'NA')
...
def normalize_table(df: pd.DataFrame):
    # Countries table
    countries_table = df[['CountryID', 'Country']].drop_duplicates().reset_index(drop=True)
    countries_table = df[['CountryID', 'Country']].groupby('CountryID').first().reset_index()
    ...
    # Transactions table
    transactions_table = df[['InvoiceNo', 'StockCode', 'Quantity', 'UnitPrice', 'TotalAmount', 'TransactionType']]

    return countries_table, invoices_table, customers_table, products_table, transactions_table
...
if __name__ == '__main__':
    ...
    countries_table, invoices_table, customers_table, products_table, transactions_table = normalize_table(df)
    print(countries_table, invoices_table, customers_table, products_table, transactions_table)